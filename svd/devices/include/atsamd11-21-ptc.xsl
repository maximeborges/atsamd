...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  <!-- Add PTC to the Generic Clock Selection ID enum on the GCLK peripheral -->
  <xsl:template name="GCLK_ID">
    <xsl:param name="VALUE" required="yes" as="xs:integer"/>
    <enumeratedValues>
      <name>IDSelect</name>
  ...
  <xsl:template name="PTC">
    <xsl:param name="BASEADDRESS" required="yes" as="xs:integer"/>
    <xsl:param name="INTERRUPT_VALUE" required="yes" as="xs:integer"/>
    <peripherals>
      ...
      <peripheral>
        ...
        <baseAddress><xsl:value-of select="$BASEADDRESS"/></baseAddress>
        <addressBlock>
          <offset>0</offset>
          <size>0x28</size>
        ...
        <interrupt>
          <name>PTC</name>
          <value><xsl:value-of select="$INTERRUPT_VALUE"/></value>
        </interrupt>
        <registers>
          <register>
            <name>CTRLA</name>
